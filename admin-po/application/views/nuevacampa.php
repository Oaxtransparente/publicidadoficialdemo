...
<tr>
<td>Tema: </td>
<td><input type="text" name="tema" id="tema" value="<?php echo set_value('tema'); ?>" /> </td>
</tr>
<tr class="columnairregular">
<td>Tipo: </td>
<td> 
...
<?php foreach($tipos->result() as $fila) { ?>		
        <option value="<?php echo $fila->tipo?>">
...
<?php foreach($clasificaciones->result() as $fila) { ?>		
        <option value="<?php echo $fila->descripcion_clasificacion?>">
		<?php echo $fila->descripcion_clasificacion?></option>        
<?php } ?>
</select>
</td>
</tr>
...
<td></td>
<td>
<div class="agregar_etiqueta" id="agregar_etiqueta" 
...
foreach($etiquetas->result() as $fila2) { ?>
         <a href="#" onClick="agregar_etiqueta_('<?php echo $fila2->etiqueta?>');">
		 <?php echo $fila2->etiqueta?></a>
<?php if($total!=$i) echo ", " ?>
<?php $i++;
} ?>
</div>
...
<tr>
<td>Objetivo: </td>
<td> <input type="text" name="objetivo" id="objetivo" value="<?php echo set_value('objetivo'); ?>" /> </td>
</tr>
<tr class="columnairregular">
<td>Inicio de la campaña: </td>
<td> <input type="text" name="publicidad_inicio" id="publicidad_inicio" class="publicidad_inicio" 
value="<?php echo set_value('publicidad_inicio'); ?>"/> <br/> </td>
</tr>
<tr>
<td>Fin de la campaña: </td>
<td> <input type="text" name="publicidad_fin" id="publicidad_fin" class="publicidad_inicio" 
value="<?php echo set_value('publicidad_fin'); ?>" /> <br> </td>
</tr>
<tr class="columnairregular">
<td>Dependencia: </td>
<td>
<select data-placeholder="seleccione dependencia" class="chzn-select" id="dependencia" 
name="dependencia" tabindex="1" style="width:155px;">
<option value="seleccione">Seleccione dependencia</option>
<?php foreach($dependencias->result() as $fila) { ?>		
        <option value="<?php echo $fila->dependencia?>">
		<?php echo $fila->dependencia?></option>        
<?php } ?>
</select>
</td>   
</tr>
<tr>
<td>Costo estimado (variable no pública): </td>
<td> <input type="text" name="costototal" id="costototal" value="<?php echo set_value('costototal'); ?>" 
onKeyPress="return validar(event)"/> <br> </td>
</tr>
<tr class="columnairregular">
<td>Estatus: </td>
<td>
...
<?php foreach($status->result() as $fila) { ?>		
        <option value="<?php echo $fila->status?>">
		<?php echo $fila->status?></option>        
<?php } ?>
</select>
</td>
</tr>
...
<td><textarea style="width:500px; height:100px;" name="video" id="video" ></textarea></td>
</tr>
<tr>
<td valign="top"></td>
<td><input type="hidden" name="video_a" id="video_a"/>
