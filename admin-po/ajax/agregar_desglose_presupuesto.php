<?php 
	...
	mysql_query("INSERT INTO tp_desglose_presupuesto_temp(id_desglose_presupuesto, id_concepto,concepto,cantidad, porcentaje) 
	VALUE (0,'$id_concepto', '$concepto', $cantidad, $porcentaje)",$db);
			
	$resultado = mysql_query("SELECT id_desglose_presupuesto, id_concepto, concepto,cantidad, porcentaje 
	FROM tp_desglose_presupuesto_temp", $db); 
	...
	while ($fila = mysql_fetch_row($resultado)){ 
	   ...
	   echo		 '<td><a class="eliminar" href="javascript:void(0);" 
	                  data-id="'.$fila[0].'" onclick="javascript:eliminar_desglose_presupuesto('.$fila[0].')" >Borrar</a> </td>
	         </tr>'; 
	}
	
	   echo '<tr>
	             <td style="color:#000; border:1px solid #c4c4c4; background:#6DA8C6;">Totales</td><td></td>
	             <td style="color:#868686; border:1px solid #c4c4c4;">$'.number_format($total).'</td>
				 <td style="color:#868686; border:1px solid #c4c4c4;">'.$total_porcentaje.'%</td>
		     </tr></table>';	
