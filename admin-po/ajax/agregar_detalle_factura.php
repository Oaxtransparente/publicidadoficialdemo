<?php 
	...
	mysql_query("INSERT INTO tp_detalle_factura_temp(concepto,unidades,monto_concepto,dependencia_s, campa_factura) 
	VALUE ('$concepto', '$unidades', '$monto',(select id_dependencia from tp_dependencia where dependencia='".$dependencia_s."'),
	(select id_campa from tp_campa where nombre='".$campa."' and  
	depen=(select id_dependencia from tp_dependencia where dependencia='".$dependencia_s."')) )",$db);
			
	$resultado = mysql_query("SELECT id_detalle, concepto, unidades, monto_concepto, dependencia, nombre 
	             FROM tp_detalle_factura_temp, tp_dependencia, tp_campa where 
				 tp_dependencia.id_dependencia=tp_detalle_factura_temp.dependencia_s and 
				 tp_campa.id_campa=tp_detalle_factura_temp.campa_factura", $db); 
	...
	while ($fila = mysql_fetch_row($resultado)){ 
       echo '<tr><td>'.$fila[1].'</td>
	             ...
	             <td><a class="eliminar" href="javascript:void(0);" data-id="'.$fila[0].'" 
				      onclick="javascript:eliminar_detalle_factura('.$fila[0].')" >Borrar</a></td>
			</tr>'; 
	}
	
	$resultado = mysql_query("SELECT SUM(unidades) as suma_unidades, sum(monto_concepto*unidades) as suma_montos 
	             FROM tp_detalle_factura_temp", $db); 
	...
	while ($fila = mysql_fetch_row($resultado)){ 
       echo '<tr><td style="color:#000; border:1px solid #c4c4c4; background:#6DA8C6;">Totales</td>
				 ...
				 <td style="color:#868686; border:1px solid #c4c4c4;">$'.number_format($fila[1]).'</td>
		     </tr>';					 		 
			 $total=$fila[1];
	}
