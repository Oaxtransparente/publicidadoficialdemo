...
function clasificacion_gastos_tipo_medio($anio){
	$query = $this->db->query("select id_clasificacion, descripcion_clasificacion, SUM(monto_total) as total from 
	tp_factura, tp_medios, tp_clasificacion 
...
function clasificacion_gastos_tipo_medioc($anio,$id){
	$query = $this->db->query("select id_clasificacion, descripcion_clasificacion, SUM(monto_total) as total from 
	tp_factura, tp_medios, tp_clasificacion where medio_id=id_medio and tp_medios.clasificacion=id_clasificacion and fecha>='".$anio."-01-01' and fecha<='".$anio."-12-31' and id_clasificacion=".$id." GROUP BY id_clasificacion;");
	$row = $query->row();
	return $row->total;			 
}
function nombre_medios_participantes($anio){
	$query = $this->db->query("select nombre_comercial,id_clasificacion,sum(monto_total) as total from 
	tp_clasificacion,tp_medios,tp_factura where id_clasificacion=clasificacion and id_medio=medio_id and fecha>='".$anio."-01-01' and fecha<='".$anio."-12-31' GROUP BY id_medio");
	return $query;
}

function costo_promedio_campa($anio){	
	$query = $this->db->query("select sum(monto_concepto)/(select count(DISTINCT campa_factura) 
from tp_detalle_factura, tp_campa where id_campa=campa_factura and anio=".$anio.") as promedio from 
tp_campa, tp_detalle_factura where id_campa=campa_factura and anio=".$anio);
...
from 
tp_campa, tp_tipo_campa, tp_clasificacion_campa, tp_dependencia, tp_detalle_factura where tp_campa.tipo=tp_tipo_campa.id_tipo and depen=id_dependencia and id_dependencia=tp_detalle_factura.dependencia_s and tp_detalle_factura.campa_factura=id_campa and clasificacion_campa=id_clasificacion_campa and id_campa=".$id." GROUP BY id_campa");
	return $query;	
}
...
function campas(){	 						
		$query = $this->db->query("select nombre, anio as año, tp_tipo_campa.tipo as tipo, objetivo, periodicidad_inicio, 
		periodicidad_fin, dependencia, costo_total as costo_aproximado,
		SUM(monto_concepto*unidades) as costo_total, tp_status_campa.status as status, descripcion_clasificacion
from tp_campa, tp_tipo_campa, tp_dependencia, tp_status_campa, tp_clasificacion_campa, tp_detalle_factura where 
tp_campa.tipo=id_tipo and depen=id_dependencia and 
tp_campa.status=id_status and tp_clasificacion_campa=id_clasificacion_campa and 
id_campa=tp_campa_factura group by tp_campa_factura
ORDER BY periodicidad_inicio desc");      			
		return $query;							
}

function contratos(){	 						
		$query = $this->db->query("select fecha_celebracion, num_contrato as numero_de_contrato, dependencia, 
		razon_social as medio, monto_contrato as monto_contratado, objeto_contrato,
fecha_inicio, fecha_fin, motivoadj as motivo_de_adjudicación, partidapres as partida_presupuestaria from 
tp_contratos, tp_dependencia, tp_medios, tp_modalidad_contratos
where depen=id_dependencia and medio=id_medio and tp_contratos.modalidad=id_modalidad");      			
		return $query;							
}
function medios(){	 						
		$query = $this->db->query("select razon_social, nombre_comercial, padron_proveedor, 
		descripcion_clasificacion as clasificacion, tp_cobertura.cobertura as cobertura 
