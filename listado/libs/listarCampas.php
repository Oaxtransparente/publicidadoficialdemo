...
$listado=  mysql_query("select id_campa, nombre, dependencia.dependencia as dependencia_solicitante, anio, 
 descripcion_clasificacion, tipo_campa.tipo as tipo, SUM(detalle_factura.monto_concepto*unidades) as total_campa
 from campa, clasificacion_campa, tipo_campa, dependencia, medios, factura, detalle_factura where 
 clasificacion_campa=id_clasificacion_campa and  campa.tipo=tipo_campa.id_tipo and
 depen=id_dependencia and medios.id_medio=factura.medio_id and 
 factura.id_factura=detalle_factura.factura and id_dependencia=dependencia_s and id_campa=campa_factura GROUP BY id_campa",$cn);
?>
                ...
                <tfoot>               
                  ...
                  <div style="margin-bottom:0px;" > </div>                                                                                            
                   ...
                   while($reg=  mysql_fetch_array($listado))
                   {
                               echo '<tr>';
                               echo '<td class="leftb"> 
							   <a href="detalle_campa/'.$reg['id_campa'].'">'.mb_convert_encoding($reg['nombre'], "UTF-8").'</a> </td>';
							   ...
							   $dependencias_contratantes=  mysql_query("select distinct dependencia from factura, detalle_factura, 
							   dependencia where id_factura=factura and dependencia_contratante=id_dependencia and 
							   campa_factura=".$reg['id_campa'],$cn);	
							   ...
							   $tipos_medios=  mysql_query("SELECT clasificacion from medios, factura, detalle_factura, clasificacion where
							    id_medio=medio_id and medios.clasificacion=clasificacion.id_clasificacion and id_factura=factura and
								campa_factura=".$reg['id_campa']." GROUP BY medio_id",$cn);														   
